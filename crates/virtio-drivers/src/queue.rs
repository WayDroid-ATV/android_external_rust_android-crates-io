...
impl<H: DeviceHal, const SIZE: usize> DeviceVirtQueue<H, SIZE> {
    ...
    pub fn poll<T>(
        ...
        let Some((buffers, token)) = self.pop_avail()? else {
            return Ok(None);
        };
    ...
    fn pop_avail<'a>(&mut self) -> Result<Option<(Vec<&'a mut [u8]>, u16)>> {
...
impl<H: DeviceHal> VirtQueueLayout<DeviceDma<H>> {
    unsafe fn map_legacy(queue_size: u16, paddr: PhysAddr, client_id: u16) -> Result<Self> {
        let (desc, avail, used) = queue_part_sizes(queue_size);
        let size = align_up(desc + avail) + align_up(used);
        let dma = unsafe { DeviceDma::new(paddr, size / PAGE_SIZE, 0, BufferDirection::Both, client_id)? };
        Ok(Self::Legacy {
            dma,
            avail_offset: desc,
            used_offset: align_up(desc + avail),
    ...
    unsafe fn map_flexible(
        ...
        let device_to_driver_dma =
            unsafe { DeviceDma::new(used_paddr, pages(used), 0, BufferDirection::DeviceToDriver, client_id)? };
        Ok(Self::Modern {
            driver_to_device_dma,
            device_to_driver_dma,
            avail_offset: desc,
