...
impl<H: DeviceHal> DeviceDma<H> {
    pub unsafe fn new(paddr: PhysAddr, pages: usize, len: usize, direction: BufferDirection, client_id: u16) -> Result<Self> {
...
impl<H: DeviceHal> DmaMemory for DeviceDma<H> {
    ...
    fn raw_slice(&self) -> NonNull<[u8]> {
        let raw_slice =
            core::ptr::slice_from_raw_parts_mut(self.vaddr(0).as_ptr(), self.len);
        NonNull::new(raw_slice).unwrap()
    }
}
...
pub trait DeviceHal {
    ...
    unsafe fn dma_map(paddr: PhysAddr, pages: usize, direction: BufferDirection, client_id: u16) -> Result<NonNull<u8>>;
