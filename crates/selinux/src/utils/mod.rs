...
pub(crate) fn str_to_c_string(the_str: &str) -> Result<CString> {
    CString::new(the_str).map_err(|_r| Error::IO1Name {
...
#[cfg(unix)]
pub(crate) fn os_str_to_c_string(os_str: &OsStr) -> Result<CString> {
    use std::os::unix::ffi::OsStrExt;

    CString::new(os_str.as_bytes()).map_err(|_r| Error::PathIsInvalid(PathBuf::from(os_str)))
}

pub(crate) fn c_str_ptr_to_str<'string>(ptr: *const c_char) -> Result<&'string str> {
    if ptr.is_null() {
...
pub(crate) fn c_str_to_non_null_ptr(c_str: &CStr) -> ptr::NonNull<c_char> {
    unsafe { ptr::NonNull::new_unchecked(c_str.as_ptr().cast_mut()) }
}
...
impl OptionalNativeFunctions {
    ...
    fn get_libselinux_handle() -> *mut c_void {
        ...
        for &lib_name in &[
            ...
            let lib_handle = unsafe { libc::dlopen(lib_name.as_ptr(), flags) };
            if !lib_handle.is_null() {
                return lib_handle;
            }
        }
    ...
    fn load_functions_addresses(&mut self, lib_handle: *mut c_void) {
        let f = unsafe { libc::dlsym(lib_handle, c"security_reject_unknown".as_ptr()) };
        if !f.is_null() {
            self.security_reject_unknown = unsafe { mem::transmute(f) };
        }

        let c_name = c"selabel_get_digests_all_partial_matches";
        let f = unsafe { libc::dlsym(lib_handle, c_name.as_ptr()) };
        if !f.is_null() {
            self.selabel_get_digests_all_partial_matches = unsafe { mem::transmute(f) };
        }

        let c_name = c"selabel_hash_all_partial_matches";
        let f = unsafe { libc::dlsym(lib_handle, c_name.as_ptr()) };
        if !f.is_null() {
            self.selabel_hash_all_partial_matches = unsafe { mem::transmute(f) };
        }

        let f = unsafe { libc::dlsym(lib_handle, c"security_validatetrans".as_ptr()) };
        if !f.is_null() {
            self.security_validatetrans = unsafe { mem::transmute(f) };
        }

        let f = unsafe { libc::dlsym(lib_handle, c"security_validatetrans_raw".as_ptr()) };
        if !f.is_null() {
            self.security_validatetrans_raw = unsafe { mem::transmute(f) };
        }

        let f = unsafe { libc::dlsym(lib_handle, c"selinux_flush_class_cache".as_ptr()) };
        if !f.is_null() {
            self.selinux_flush_class_cache = unsafe { mem::transmute(f) };
        }

        let f = unsafe { libc::dlsym(lib_handle, c"selinux_restorecon_parallel".as_ptr()) };
        if !f.is_null() {
            self.selinux_restorecon_parallel = unsafe { mem::transmute(f) };
        }

        let c_name = c"selinux_restorecon_get_skipped_errors";
        let f = unsafe { libc::dlsym(lib_handle, c_name.as_ptr()) };
        if !f.is_null() {
            self.selinux_restorecon_get_skipped_errors = unsafe { mem::transmute(f) };
        }

        let f = unsafe { libc::dlsym(lib_handle, c"getpidprevcon".as_ptr()) };
        if !f.is_null() {
            self.getpidprevcon = unsafe { mem::transmute(f) };
        }

        let f = unsafe { libc::dlsym(lib_handle, c"getpidprevcon_raw".as_ptr()) };
        if !f.is_null() {
