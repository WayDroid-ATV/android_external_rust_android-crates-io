...
`./crate_tool updatable-crates` will print every newer version of every crate.

### Analyzing updates for a particular crate

`./crate_tool analyze-updates <crate name>` will check all newer versions of the crate for potential problems.

### Updating a crate

`./crate_tool update <crate name> <version>` will try to update the crate to the specified version.
